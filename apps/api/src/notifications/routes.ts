...
export function buildNotificationRoutes(db: Kysely<DB>, config: AppConfig): FastifyPluginAsync {
  ...
  return async (fastify) => {
    ...
    fastify.get("/api/notifications/preferences", { preHandler: [requireAuth] }, async (request) => {
      const preferences = await resolveAllPreferences(db, request.authUser!.id);
    ...
    fastify.put(
      ...
      async (request, reply) => {
        ...
        const preferences = await resolveAllPreferences(db, userId);
        return { data: { preferences } };
      },
    );
  };
