...
export interface BunnyInternalApiTestDoubleState {
  ...
  /** When set, overrides the response status/body UNCONDITIONALLY (ignores auth/guild lookup entirely) — simulates a non-200 upstream error or a malformed 200 body. */
...
export async function startBunnyInternalApiTestDouble(
  ...
  const server = http.createServer((req, res) => {
    const url = req.url ?? "";
    const match = /^\/internal\/guilds\/([^/]+)\/channels$/.exec(url);
